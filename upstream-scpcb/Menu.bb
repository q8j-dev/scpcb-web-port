	...
	Else
		...
		Select MainMenuTab
			...
			Case 2 ;load game
				...
				Text(x+(width/2.0),y+PagingFrameHeight+26*MenuScale,Format3(I_Loc\Menu_Page, Int(Max((CurrLoadGamePage+1),1)), Int(Max((Int(Ceil(Float(SaveGameAmount)/EntriesPerPage))),1))),True,True)
			...
			Case 4 ; load map
				...
				Text(x+(width/2.0),y+PagingFrameHeight+26*MenuScale,Format3(I_Loc\Menu_Page, Int(Max((CurrLoadGamePage+1),1)), Int(Max((Int(Ceil(Float(SavedMapsAmount)/EntriesPerPage))),1))),True,True)
			...
			Case 8 ;Mods
				...
				Text(x+(width/2.0),y+PagingFrameHeight+26*MenuScale,Format3(I_Loc\Menu_Page, Int(Max((CurrLoadGamePage+1),1)), Int(Max((Int(Ceil(Float(ModCount)/entriesPerPage))),1))),True,True)
	...
	Repeat 
		...
		If SelectedLoadingScreen\title = "CWM" Then
			...
			If Not shortloading Then 
				If firstloop Then 
					If percent = 0 Then
						LoadingScreenCHN = PlaySound_Strict(LoadTempSound("SFX\SCP\990\cwm1.ogg"))
						LoadingScreenCWM = 0
					EndIf
				EndIf
			EndIf
			...
			If (Not shortloading) And percent = 100 And (Not ChannelPlaying(LoadingScreenCHN)) And LoadingScreenCWM = 0
				LoadingScreenCHN = PlaySound_Strict(LoadTempSound("SFX\SCP\990\cwm2.ogg"))
				LoadingScreenCWM = 1
			EndIf
...
Function RowText(A$, X, Y, W, H, align% = 0, Leading#=1)
	...
	While Len(A$) > 0
		...
		If StringWidth (b$ + temp$) > W And b$ <> "" Then ;too big, so Print what will fit
			If align Then
...
Function DrawOptionsTooltip(x%,y%,width%,height%,option$,value#=0,ingame%=False)
	...
	Select Lower(option$)
		...
		Case "gamma"
			...
			G = 255
			B = 255
			txt2 = Format4(I_Loc\Option_HintDefault, "%", Str(Int(value*100)), "100")
		Case "texquality"
		...
		Case "hudoffset"
			...
			G = 255
			B = 255
			txt2 = Format4(I_Loc\Option_HintDefault, "%", Str(Int(HUDOffsetScale*100)), "0")
		Case "viewbob"
			txt = I_Loc\OptionTooltip_Viewbob
			R = 255
			G = 255
			B = 255
			txt2 = Format4(I_Loc\Option_HintDefault, "%", Str(Int(ViewBobScale*100)), "100")
		Case "fov"
			txt = I_Loc\OptionTooltip_Fov
			R = 255
			G = 255
			B = 255
			txt2 = Format4(I_Loc\Option_HintDefault, "°", Str(FOV), Str(DEFAULT_FOV))
			;[End Block]
		;Sound options
			;[Block]
		Case "musicvol"
			...
			G = 255
			B = 255
			txt2 = Format4(I_Loc\Option_HintDefault, "%", Str(Int(value*100)), "50")
		Case "soundvol"
		...
		Case "mousesensitivity"
			...
			G = 255
			B = 255
			txt2 = Format4(I_Loc\Option_HintDefault, "%", Str(Int((0.5+value)*100)), "50")
		Case "mouseinvert"
