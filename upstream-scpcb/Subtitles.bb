...
Function UpdateSubtitles(factor#)
	...
	For c.SubtitleMsg = Each SubtitleMsg
		...
		Else
			...
			c\curYPos = CurveValue4(yPos, c\curYPos, 7.0, factor)
		EndIf
	Next

	RecalculateSubtitleBoxTarget()
	If SubBox\lines>0 Then
		SubBox\curHeight = CurveValue4(SubBox\targetHeight, SubBox\curHeight, 7.0, factor)
		SubBox\curTop = (SubBox\screenTop + SubtitleTextHeight) - SubBox\curHeight

		SubBox\alpha = CurveValue4(1.0, SubBox\alpha, 7.0, factor)
	Else
		SubBox\alpha = CurveValue4(0.0, SubBox\alpha, 7.0, factor)
	EndIf
End Function
