...
Global LastOptionsSignature$ = ""
Global SettingsSavedToastTimer# = 0
...
If GraphicWidth <= 0 Then
	GraphicWidth = DesktopWidth()
	GraphicHeight = DesktopHeight()
	If GraphicWidth > 1920 And GraphicHeight > 1080 Then
		GraphicWidth = 1920
		GraphicHeight = 1080
	EndIf
EndIf
...
Function MovePlayer()
	...
	If KeyHit(KEY_CROUCH) And Playable Then Crouch = 1 - Crouch
...
Function CreateOverlayQuad%(parent%)
	Local m% = CreateMesh(parent)
	Local sf% = CreateSurface(m)
	...
	EntityFX m, 17
	ScaleEntity m, 1.0, Max(Float(GraphicHeight) / Float(GraphicWidth), 0.8), 1
	MoveEntity m, 0, 0, 1.0
	Return m
End Function

Function CreatePersistentOverlays()
	...
	Fog = CreateOverlayQuad(ark_blur_cam)
	EntityTexture(Fog, FogTexture)
	EntityBlend (Fog, 2)
	...
	GasMaskOverlay = CreateOverlayQuad(ark_blur_cam)
	...
	InfectOverlay = CreateOverlayQuad(ark_blur_cam)
	...
	NVOverlay = CreateOverlayQuad(ark_blur_cam)
	EntityTexture(NVOverlay, NVTexture)
	EntityBlend (NVOverlay, 2)
	EntityOrder NVOverlay, -1003
	HideEntity(NVOverlay)
	NVBlink = CreateOverlayQuad(ark_blur_cam)
	...
	Dark = CreateOverlayQuad(ark_blur_cam)
	EntityTexture(Dark, DarkTexture)
	EntityColor(Dark, 0, 0, 0)
	EntityBlend (Dark, 1)
	EntityOrder Dark, -1002
	EntityAlpha Dark, 0.0

	GammaOverlay = CreateOverlayQuad(ark_blur_cam)
	ScaleEntity GammaOverlay, Max(2.4, Float(GraphicWidth) / 620.0), Max(2.4, Float(GraphicHeight) / 480.0), 1
	EntityOrder GammaOverlay, -1003
	EntityAlpha GammaOverlay, 0.0
	EntityBlend GammaOverlay, 1
	...
	Light = CreateOverlayQuad(ark_blur_cam)
...
Function CurveValue4#(number#, old#, smooth#, factor#)
...
Function CurveValue#(number#, old#, smooth#)
	Return CurveValue4(number, old, smooth, FPSfactor)
End Function
...
Function OptionsSignature$()
	Local s$ = ""
	s = s + MouseSens + "|" + InvertMouse + "|" + HUDenabled + "|" + ScreenGamma + "|" + Opt_AntiAlias + "|"
	s = s + Vsync + "|" + ShowFPS + "|" + Framelimit% + "|" + AchvMSGenabled% + "|" + LauncherEnabled% + "|"
	s = s + TextureDetails% + "|" + CanOpenConsole% + "|" + ConsoleOpening% + "|" + SpeedRunMode% + "|"
	s = s + UseNumericSeeds% + "|" + EnableVRam + "|" + MouseSmooth + "|" + HUDOffsetScale + "|"
	s = s + ViewBobScale + "|" + FOV + "|" + MusicVolume + "|" + PrevSFXVolume + "|" + EnableSFXRelease + "|"
	s = s + EnableUserTracks% + "|" + UserTrackMode% + "|" + DubbedAudio + "|" + SubtitlesEnabled + "|"
	s = s + ClosedCaptionsEnabled + "|" + KEY_RIGHT + "|" + KEY_LEFT + "|" + KEY_UP + "|" + KEY_DOWN + "|"
	s = s + KEY_BLINK + "|" + KEY_SPRINT + "|" + KEY_INV + "|" + KEY_CROUCH + "|" + KEY_SAVE + "|" + KEY_CONSOLE
	Return s
End Function

Function AutoSaveOptionsIfChanged()
	Local sig$ = OptionsSignature()
	If LastOptionsSignature = "" Then
		LastOptionsSignature = sig
		Return
	EndIf
	If sig <> LastOptionsSignature Then
		LastOptionsSignature = sig
		SaveOptionsINI()
		SettingsSavedToastTimer# = 2.0
	EndIf
End Function

Function DrawSettingsSavedToast()
	If SettingsSavedToastTimer <= 0 Then Return
	SettingsSavedToastTimer = SettingsSavedToastTimer - (FPSfactor/70.0)
	SetFont Font1
	Local tw# = StringWidth(I_Loc\Menu_SettingsSaved)
	Local tx# = GraphicWidth - tw - 20*MenuScale
	Local ty# = GraphicHeight - 40*MenuScale
	Color 0,0,0
	Text(tx+Max(1,MenuScale), ty+Max(1,MenuScale), I_Loc\Menu_SettingsSaved)
	Color 255,255,255
	Text(tx, ty, I_Loc\Menu_SettingsSaved)
End Function
...
;not by any means a perfect solution
;Not even proper gamma correction but it's a nice looking alternative that works in windowed mode
;implemented as a camera-attached fullscreen sprite blended into the normal scene pass (like the "Dark" blink overlay),
;rather than the old capture-backbuffer-then-rerender trick which needs a working mid-frame render-to-texture round trip
Function ApplyGammaOverlay()
	If GammaOverlay = 0 Then Return

	If ScreenGamma > 1.0 Then
		EntityColor GammaOverlay, 255,255,255
		EntityBlend GammaOverlay, 3
		EntityAlpha GammaOverlay, Min(ScreenGamma - 1.0, 1.0)
	ElseIf ScreenGamma < 1.0 Then
		EntityColor GammaOverlay, 0,0,0
		EntityBlend GammaOverlay, 1
		EntityAlpha GammaOverlay, Min(1.0 - ScreenGamma, 1.0)
	Else
		EntityAlpha GammaOverlay, 0.0
	EndIf
End Function
...
Function PlayMovie(moviefile$)
	...
	Local playingVideo% = True
	Local frame% = 0
	Repeat
		Cls
		playingVideo = DrawMovie(SplashScreenVideo, 0, (RealGraphicHeight/2-ScaledGraphicHeight/2), RealGraphicWidth, ScaledGraphicHeight)
		Flip
		frame = frame + 1
	Until (GetKey() Or frame>900 Or ((Not playingVideo) And (Not IsStreamPlaying_Strict(SplashScreenAudio))))
